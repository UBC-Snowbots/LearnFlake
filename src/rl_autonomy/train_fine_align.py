#!/usr/bin/env python3
"""
Phase 6/7 — FineAlign skill training.

Trains a SAC policy to precisely align the EEF tip within 5mm above the target
key, starting from CoarseReach's end state (~3cm away). Uses proprioceptive
observations + synthesized ArUco (dx, dy, key_visible).

Usage:
    python train_fine_align.py                        # train from scratch
    python train_fine_align.py --eval checkpoints/fine_align/best_model.zip
    python train_fine_align.py --resume checkpoints/fine_align/last_model.zip
    python train_fine_align.py --timesteps 300000 --no-render
"""
...
from keyboard_env import FineAlignEnv

CHECKPOINT_DIR = os.path.join(ROOT, "checkpoints", "fine_align")
LOG_DIR        = os.path.join(ROOT, "logs", "fine_align")
...
class FineAlignGymEnv(gym.Env):
    """
    Gym/Gymnasium wrapper around FineAlignEnv for SB3 compatibility.
    ...
    def __init__(self, render: bool = False, random_key: bool = True):
        super().__init__()
        self._env = FineAlignEnv(render=render, random_key=random_key)
    ...
    def step(self, action):
        # Scale actions to 30% — fine alignment needs small, precise movements
        full_action = np.append(action * 0.3, 0.0)   # lock solenoid retracted
        ...
        z_error = float(abs((eef_pos[2] - key_pos[2]) - FineAlignEnv.HOVER_HEIGHT))
        tilt    = float(FineAlignEnv._eef_tilt_from_vertical(eef_quat))
...
def make_env(render: bool = False):
    def _init():
        env = FineAlignGymEnv(render=render)
        return Monitor(env)
    return _init
...
def train(timesteps: int, render: bool, resume_path: str | None,
    ...
    callbacks = CallbackList([
        ...
        CheckpointCallback(
            ...
            name_prefix="fine_align",
            verbose=1,
        ),
    ])

    print(f"\nTraining FineAlign for {timesteps:,} steps ({num_envs} parallel envs)")
...
def main():
    ...
    parser.add_argument('--timesteps', type=int, default=200_000,
                        help='Total training steps (default: 200k)')

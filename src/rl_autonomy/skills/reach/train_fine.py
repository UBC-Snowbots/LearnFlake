...
Precisely center EEF within 5mm above target key, starting from CoarseReach
output (~3cm scatter). Uses ArUco obs for precision. Solenoid locked. 6-dim
arm action scaled to 30%.

Usage:
    python -m skills.reach.train_fine
    python -m skills.reach.train_fine --eval checkpoints/reach_fine/best_model.zip
"""
...
class FineReachGymEnv(gym.Env):
    ...
    def step(self, action):
        full_action = np.append(action * 0.3, 0.0)
        ...
        info['z_error']    = float(abs((eef_pos[2] - key_pos[2]) - FineAlignEnv.HOVER_HEIGHT))
        info['tilt_rad']   = float(FineAlignEnv._eef_tilt_from_vertical(eef_quat))
...
def _print_eval(ep, n, reward, info, success):
    ...
    print(f"  ep {ep+1:3d}/{n}  key={key}  reward={reward:7.1f}  "
          f"xy={xy*100:5.2f}cm  z_err={ze*100:5.2f}cm  tilt={np.degrees(tlt):5.1f}°  {s}")


if __name__ == '__main__':
    run_cli(FineReachGymEnv, "reach_fine", default_timesteps=200_000,

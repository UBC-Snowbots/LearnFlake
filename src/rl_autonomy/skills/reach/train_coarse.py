...
Move EEF to within 3cm XY and correct hover height above any keyboard key.
Solenoid locked retracted. 6-dim arm joint action space.

Usage:
    python -m skills.reach.train_coarse
    python -m skills.reach.train_coarse --eval checkpoints/reach_coarse/best_model.zip
    python -m skills.reach.train_coarse --timesteps 200000 --no-render
"""
...
class CoarseReachGymEnv(gym.Env):
    ...
    def step(self, action):
        ...
        info['z_error']    = float(abs((eef_pos[2] - key_pos[2]) - CoarseReachEnv.HOVER_HEIGHT))
        info['tilt_rad']   = float(CoarseReachEnv._eef_tilt_from_vertical(eef_quat))
...
if __name__ == '__main__':
    run_cli(CoarseReachGymEnv, "reach_coarse", default_timesteps=150_000,

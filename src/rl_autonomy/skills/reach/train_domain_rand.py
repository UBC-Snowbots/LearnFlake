#!/usr/bin/env python3
"""
Reach — Domain randomization stage.

Same as fine reach but wrapped in DomainRandWrapper for sim-to-real transfer.
Randomizes keyboard position, gravity, friction, obs noise, action scaling,
and control latency on every reset.

Usage:
    python -m skills.reach.train_domain_rand
    python -m skills.reach.train_domain_rand --eval checkpoints/reach_dr/best_model.zip
"""
...
from keyboard_env import FineAlignEnv, DomainRandWrapper


class DRReachGymEnv(gym.Env):
    metadata = {'render_modes': ['human']}

    def __init__(self, render: bool = False, random_key: bool = True):
        super().__init__()
        base = FineAlignEnv(render=render, random_key=random_key)
        self._dr = DomainRandWrapper(base)
        self._env = base  # for _check_success access
...
if __name__ == '__main__':
    run_cli(DRReachGymEnv, "reach_dr", default_timesteps=300_000,

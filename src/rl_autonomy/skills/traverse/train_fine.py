...
Precise key-to-key movement with 1cm XY tolerance (vs 3cm coarse).
Penalizes jerk for smooth handoff to FineAlign. 6-dim arm action,
solenoid locked retracted.

Usage:
    python -m skills.traverse.train_fine
    python -m skills.traverse.train_fine --eval checkpoints/traverse_fine/best_model.zip
"""
...
from keyboard_env import FineTraverseEnv


class FineTraverseGymEnv(gym.Env):
    metadata = {'render_modes': ['human']}

    def __init__(self, render: bool = False, random_key: bool = True):
        super().__init__()
        self._env = FineTraverseEnv(render=render, random_key=random_key)
    ...
    def step(self, action):
        ...
        info['tilt_rad']   = float(FineTraverseEnv._eef_tilt_from_vertical(eef_quat))
...
def _print_eval(ep, n, reward, info, success):
    ...
    print(f"  ep {ep+1:3d}/{n}  {src}->{tgt}  reward={reward:7.1f}  "
          ...
          f"tilt={np.degrees(tlt):5.1f}°  {s}")


if __name__ == '__main__':
    run_cli(FineTraverseGymEnv, "traverse_fine", default_timesteps=250_000,

...
RL_Sim::RL_Sim(int argc, char **argv)
{
...
#if defined(USE_ROS2)
    this->use_cartesian_bridge_ = (this->robot_name == "rover2026" &&
                                   this->params.Get<std::string>("control_bridge_mode") == "cartesian_moveit");
    if (this->params.Has("cartesian_cmd_topic"))
    {
        this->cartesian_cmd_topic_ = this->params.Get<std::string>("cartesian_cmd_topic");
    }
    if (this->params.Has("cartesian_cmd_frame"))
    {
        this->cartesian_cmd_frame_ = this->params.Get<std::string>("cartesian_cmd_frame");
    }
    if (this->cartesian_cmd_frame_.empty())
    {
        this->cartesian_cmd_frame_ = "base_link";
    }
#endif
    ...
    if (this->use_cartesian_bridge_)
    {
        this->cartesian_cmd_pub_ = ros2_node->create_publisher<geometry_msgs::msg::TwistStamped>(
            this->cartesian_cmd_topic_, rclcpp::SystemDefaultsQoS());
        std::cout << LOGGER::INFO << "[RoverBridge] Cartesian MoveIt mode enabled. Topic: "
                  << this->cartesian_cmd_topic_ << " frame: " << this->cartesian_cmd_frame_ << std::endl;
    }
...
void RL_Sim::RunModel()
{
    if (this->rl_init_done && simulation_running)
    {
        ...
        std::vector<float> model_actions = this->Forward();

#if defined(USE_ROS2)
        if (this->use_cartesian_bridge_)
        {
            static int cartesian_action_warn_count = 0;
            this->obs.actions.assign(this->params.Get<int>("num_of_dofs"), 0.0f);
            for (size_t i = 0; i < std::min<size_t>(3, this->obs.actions.size()) && i < model_actions.size(); ++i)
            {
                this->obs.actions[i] = model_actions[i];
            }

            geometry_msgs::msg::TwistStamped twist_msg;
            twist_msg.header.stamp = ros2_node->now();
            twist_msg.header.frame_id = this->cartesian_cmd_frame_.empty() ? "base_link" : this->cartesian_cmd_frame_;
            ...
            if (model_actions.size() == 3)
            {
                twist_msg.twist.linear.x = model_actions[0];
                twist_msg.twist.linear.y = model_actions[1];
                twist_msg.twist.linear.z = model_actions[2];
            }
            else if (cartesian_action_warn_count % 200 == 0)
            {
                std::cout << LOGGER::WARNING << "[RoverBridge] Expected 3 cartesian actions, got "
                          << model_actions.size() << ". Publishing zero twist." << std::endl;
                ++cartesian_action_warn_count;
            }
            else
            {
                ++cartesian_action_warn_count;
            ...
            if (this->cartesian_cmd_pub_)
            {
                this->cartesian_cmd_pub_->publish(twist_msg);
            }
            return;
        }
#endif

        this->obs.actions = model_actions;

        if (model_actions.size() != static_cast<size_t>(this->params.Get<int>("num_of_dofs")))
        {
            static int legacy_action_warn_count = 0;
            if (legacy_action_warn_count % 200 == 0)
            {
                std::cout << LOGGER::WARNING << "[RunModel] Action size mismatch. Expected "
                          << this->params.Get<int>("num_of_dofs") << ", got " << model_actions.size()
                          << ". Skipping this control tick." << std::endl;
            }
            ++legacy_action_warn_count;
            return;
        }
